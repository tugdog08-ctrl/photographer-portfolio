...
function App() {
  ...
  return <main>
    ...
    <section className="hero" id="home">
      <img className="hero-image" src={composites[3].image} alt="后期合成作品：危城红影"/>
    ...
    <section className="intro section-wrap" id="about">
      ...
      <div className="profile-strip">
        <figure><img src={profile.portrait} alt="摄影师与拍摄器材工作照"/></figure>
    ...
    <section className="field-notes section-wrap" id="works">
      ...
      <div className="field-grid">{fieldNotes.map(photo => <figure key={photo.src}><img src={photo.src} alt={photo.alt}/><figcaption><span>{photo.index} / ON LOCATION</span><span>AVAILABLE LIGHT + FLASH</span></figcaption></figure>)}</div>
    </section>

    <section className="selected section-wrap">
      <div className="chapter"><span>03</span><span>SELECTED PHOTOGRAPHS / 精选外景</span></div>
      <div className="selected-grid">{projects.map(project => <figure key={project.no}><img src={project.image} alt={project.title}/><figcaption><span>{project.no}</span><strong>{project.title}</strong><small>{project.tag}</small></figcaption></figure>)}</div>
    </section>
    ...
    <section className="post section-wrap" id="postproduction">
      ...
      <div className="post-grid">{composites.map(item => <figure className={item.format} key={item.no}><img src={item.image} alt={`后期合成作品：${item.title}`}/><figcaption><span>{item.no} / POST-PRODUCTION</span><strong>{item.title}</strong></figcaption></figure>)}</div>
    </section>
